class FilamentManager {
    ...
    updateSelects() {
        ...
        // También actualizar selectores de multicolor si existen
        this.updateMulticolorSelects();
        
        // También llamar al inicializador del calculator si existe
        if (window.calculator && typeof window.calculator.inicializarSelectoresFilamentos === 'function') {
            window.calculator.inicializarSelectoresFilamentos();
        }
    }

    // Nueva función para actualizar selectores multicolor
    updateMulticolorSelects() {
        // Actualizar selectores de multicolor individual
        const multiSelects = document.querySelectorAll('.filamento-multicolor-individual');
        multiSelects.forEach(select => {
            this.populateSelect(select);
        });

        // Actualizar selectores de piezas múltiples (single)
        const pieceSingleSelects = document.querySelectorAll('.filamento-pieza-single');
        pieceSingleSelects.forEach(select => {
            this.populateSelect(select);
        });

        // Actualizar selectores de piezas múltiples (multicolor)
        const pieceMultiSelects = document.querySelectorAll('.filamento-multicolor-pieza');
        pieceMultiSelects.forEach(select => {
            this.populateSelect(select);
        });
    }

    // Función auxiliar para popular un selector específico
